...
class VertexAdapter(AdapterBase):
    def __init__(self):
        self._client = None
    ...
    def _initialize_client(self):
        if self._client is None:
            ...
            if not VERTEX_API_KEY:
                raise ValueError("VERTEX_API_KEY is not set.")

            self._client = genai.Client(
                api_key=VERTEX_API_KEY,
                vertexai=True,
            )
            logger.info(f"Created Vertex AI client")
        ...
        **kwargs
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.info(f"Requesting completion from Vertex AI with model {model} (attempt {attempt + 1}/{max_retries + 1})")
                ...
                if response and response.text:
                    ...
                    if hasattr(response, 'usage_metadata') and response.usage_metadata:
                        ...
                        logger.warning("No usage metadata available from Vertex AI response")
                    
                    return response_text, prompt_tokens, completion_tokens, total_tokens, latency
                else:
                    response_text = "Error: No response text received from Vertex AI"
                ...
                if is_rate_limit and attempt < max_retries:
                    ...
                    response_text = f"Error with Vertex AI: {type(exception).__name__}: {exception}"
                    logger.error(response_text)
        ...
        if response_text is None:
            response_text = f"Error with Vertex AI after {max_retries} retries: {type(last_exception).__name__}: {last_exception}"
